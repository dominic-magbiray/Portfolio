/*
We started with joining the columns to see every essential data for cleaning, transformation, exploration, and analysis.
...
ROUND(
SUM(research_development_spent),2) as Research_Dev,
ROUND (
SUM(profit),2)as profit,
ROUND(
SUM(marketing_spent),2)as marketing_spent,
ROUND(
SUM(population.estimate),2)as population, 
ROUND(
AVG(convictions_per_capita),2)AS conviction_rate,
ROUND(
AVG(avg_spending),2)as health_spending,
ROUND(
AVG(avg_price) ,2)as avg_property_price,
ROUND(
AVG(average_income) ,2)as avg_state_income

from health_spending
full join population
on population.state_usa = health_spending.state_usa
full join corruption_convictions_per_capita
on corruption_convictions_per_capita.state_usa = health_spending.state_usa
full join competitors
on health_spending.state_usa = competitors.state_usa
full join property_prices
on health_spending.state_usa = property_prices.state_usa
full join state_income
...
SELECT DISTINCT state_usa,estimate FROM population;
...
FROM state_income
JOIN corruption_convictions_per_capita
ON corruption_convictions_per_capita.state_usa = state_income.state_usa
